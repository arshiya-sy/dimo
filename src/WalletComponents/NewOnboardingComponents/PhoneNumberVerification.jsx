...
class PhoneNumberVerification extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            seconds: 150,
            showResend: false,
            step: this.props.location.state && this.props.location.state.step ? this.props.location.state.step : "",
            open: false,
            snackBarOpen: false,
            phoneNum: this.props.location.state && this.props.location.state.phoneNumber ? this.props.location.state.phoneNumber : "",
            ddd: this.props.location.state && this.props.location.state.ddd ? this.props.location.state.ddd : "",
            whatsappOptIn: this.props.location.state && this.props.location.state.whatsappOptIn ? this.props.location.state.whatsappOptIn : "",
            displayNumber: this.props.location.state && this.props.location.state.displayNumber ? this.props.location.state.displayNumber : "",
    ...
    componentDidMount() {
        ...
        if (this.props.location.state?.cpf) {
    ...
    sendField = () => {
        ...
        } else if (this.state.fromForgetPassword) {
            // this.props.history.replace("/phoneNumberVerification", { from: "forgetPassword" });
            this.props.history.replace({
                pathname: "/userIdCreation",
                state: { 
                    creationState: "password", 
                    from: "forgetPassword", 
                    forgetPasswordOTP: otpValue, 
                    cpf: this.state.cpf
                },
            });
        } else {
            this.verifyOtpToken(otpValue);
        }
    }


    verifyOtpToken = (value) => {
        this.showProgressDialog();
        ArbiApiService.validateOTPToken(value).then(response => {
            this.hideProgressDialog();
            if (response.success) {
                let jsonObj = {};
                jsonObj["mobileNumber"] = this.props.location.state.phoneNumber;
                jsonObj["ddd"] = this.props.location.state.ddd;
                arbiApiService.createClientPayloadJson("userInfo", jsonObj)
    ...
    onBack = () => {
        ...
        if (this.state.processing) {
            ...
            return this.props.history.replace({
                pathname: "/userIdCreation",
                state: { creationState: "phoneNumber",
                    phoneNumber: this.state.phoneNum,
                    ddd: this.state.ddd,
                    displayNumber: this.state.displayNumber,
                    step: this.state.step - 1,
                    cpf: this.props.location && this.props.location.state && this.props.location.state.cpf ? this.props.location.state.cpf  : "",
                    name: this.props.location && this.props.location.state && this.props.location.state.name ? this.props.location.state.name : "",
                    email: this.props.location && this.props.location.state && this.props.location.state.email ? this.props.location.state.email : ""
                },
                transition: "right"
            });
        }
    ...
    onChangeNumber = () => {
        ...
        this.props.history.replace({
            pathname: "/userIdCreation",
            state: { creationState: "phoneNumber", phoneNumber: this.state.phoneNum, step: this.state.step - 1 },
            transition: "right"
        });
        this.setState({ open: false })
    }
    ...
    resend = () => {
        ...
        this.showProgressDialog();
        let jsonObj = {};
        jsonObj["phoneNumber"] = this.props.location.state.phoneNumber;
        jsonObj["ddd"] = this.props.location.state.ddd;
        ArbiApiService.sendingOTPToken(jsonObj).then(response => {
            this.hideProgressDialog();
            if (response.success) {
                this.setState({
                    seconds: 150,
    ...
    render() {
        ...
        return (
            <div>
                <ButtonAppBar header={this.state.fromForgetPassword ? localeObj.otp_header : localeObj.acc_verification} onBack={this.onBack} onCancel={this.onCancel} action="cancel" />
                    ...
                    <div className="scroll" style={{ height: this.state.fieldOpen ? `${finalHeight - 470}px` : `${finalHeight - 280}px`, overflowY: "auto", overflowX: "hidden" }}>
                        <div style={InputThemes.initialMarginStyle}>
                            <FlexView column>
                                ...
                                <div className="body2 highEmphasis" style={{ textAlign: "left", marginTop: "0.6rem" }}>
                                    {this.state.fromForgetPassword ? localeObj.phone_otp_description : localeObj.phone_otp_description + " " + "55 " + this.state.ddd + " " + this.state.phoneNum + " " + localeObj.phone_otp_description2}
                                </div>
                            </FlexView>
                        </div>
                        ...
                        <div style={{ display: (this.state.showResend ? 'block' : 'none') }}>
                            <FlexView column>
                                ...
                                {this.state.whatsappOptIn !== "undecided" &&
                                    <div>
                                        <div className="label2 highEmphasis" style={{ textAlign: "center", padding: "10px" }}> {localeObj.otp_or} </div>
                                        <div className="label2 accent" style={{ textAlign: "center" }} >
                                            {localeObj.whatsApp_code}
                                        </div>
                                    </div>
                                }
                            </FlexView>
                        ...
                        <FlexView column hAlignContent="center" style={this.state.fieldOpen ? InputThemes.formButtonStyle : InputThemes.bottomButtonStyle}>
                            ...
                            {!this.state.fromForgetPassword && <SecondaryButtonComponent btn_text={localeObj.change_phone_number} onCheck={this.onChangeNumber} />}
                        </FlexView>
                    </div>
                </div>
                <div style={{ display: this.state.processing ? 'block' : 'none' }}>
...
export default withStyles(styles)(PhoneNumberVerification);
